...
class CSerialPort : public INotify
{
public:
	CSerialPort(void);
	~CSerialPort(void);
     ...
     *  @see:      
     */ 
    BOOL ReadChar(char *cRecved, DWORD &len);  
 
	/**
	* @brief: 从串口读取数据
    * @description:	从串口读取数据
	* @param: Buffer 数据缓存区
	* @param: dwBufferLength 读取数据长度
	* @param: dwWaitTime 读取数据时间限制
	* @return: 实际读取数据长度
	*/	
	int ReadBuf(LPVOID Buffer, DWORD dwBufferLength, DWORD dwWaitTime = 10);
public:
		/**
	* @brief: 添加串口数据观察者
	...
	* @return: N/A
	*/	
	virtual void AddObserver(IDataUpdate *pObserver);
